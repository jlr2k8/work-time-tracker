...
def estimate_hours_from_commits(commits: List[Dict]) -> float:
	...
	for date, day_commits in commits_by_date.items():
		...
		merge_commits = [c for c in day_commits if 'merge' in c.get('message', '').lower()]
...
Examples:
  ...
  python track_work.py ../myproject 2024-11-10 --invoice --invoice-date 11/20/24
	...
	parser.add_argument('--invoice-output', type=str, default='.',
					   help='Directory to save invoice PDF (default: current directory)')
